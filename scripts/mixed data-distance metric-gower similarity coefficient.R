# Script objective: Gower similarity coeffecient
# script create date: 25/3/2019
# script last modified on: 25/3/2019

# What is Gower similarity coeffecient?
# Gower similarity coefficient which is a composite measure1; it takes quantitative (such as rating scale), binary (such as present/absent) and nominal (such as worker/teacher/clerk) variables. Later Podani2 added an option to take ordinal variables as well.
# How it works- you compute the similarity value between the individuals by each variable, taking the type of the variable into account, and then average across all the variables.
## When all variables are quantitative (interval) then the coefficient is the range-normalized Manhattan distance converted into similarity. 
...
## When all variables are nominal (also including here dichotomous with symmetric significance: "this" vs "that") then the coefficient is the Dice matching coefficient that you obtain from your nominal variables if recode them into dummy variables
## The Gower coeffecient ranges between 0 and 1.

# References
# 0. https://github.com/pimentel/cluster/blob/master/R/daisy.q
# 1. https://stats.stackexchange.com/questions/15287/hierarchical-clustering-with-mixed-type-data-what-distance-similarity-to-use
# 2. Gower J. C. A general coefficient of similarity and some of its properties // Biometrics, 1971, 27, 857-872
# 3. Podani, J. Extending Gower’s general coefficient of similarity to ordinal characters // Taxon, 1999, 48, 331-340 
